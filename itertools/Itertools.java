...
public class Itertools {
    ...
    public static <T> Iterator<T> take(Iterator<T> it, int count) {
        ...
        return new Iterator<T>() {
            private int remainnumber = count;

            @Override
            public boolean hasNext() {
                if (remainnumber <= 0) {
                    // Returns false if no more elements are to be taken
                    return false;
                } else {
                    // Otherwise, check if there are more elements in the original iterator
                    return it.hasNext();
                }
            }
            ...
            public T next() {
                if (!hasNext()) {
                    // Throws an exception if there are no elements available to take
                    throw new NoSuchElementException("No more elements available to take.");
                }
                remainnumber--;
                return it.next();
            }
    ...
    public static <T, R> DoubleEndedIterator<R> map(DoubleEndedIterator<T> it, Function<T, R> f) {
        ...
        return new DoubleEndedIterator<R>() {
            ...
            public R reverseNext() {
                ...
                return f.apply(iterator.reverseNext());
            }
        };
    }
